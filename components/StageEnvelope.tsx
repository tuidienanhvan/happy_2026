...
const StageEnvelope: React.FC<Props> = ({ onForgive }) => {
  ...
  // State to delay wrapper visibility (prevent FOUC)
  const [showWrapper, setShowWrapper] = useState(false);

  // Delay showing wrapper effects
  useEffect(() => {
  ...
  const handleOpenSequence = () => {
    if (step > 0) return;
    playSound('open'); // Envelope opening sound
    setStep(1);
    setTimeout(() => setStep(2), 900);
    setTimeout(() => {
      playSound('whoosh'); // Letter sliding out
      setStep(3);
    }, 1800);
  };
  ...
  const handleLetterClick = () => {
    if (step === 3) {
      playSound('click'); // Letter click to read
      setStep(4);
    }
  };
  ...
  const handleForgive = () => {
    ...
    setIsFlipped(true);
    setTimeout(() => playSound('success'), 600); // Success chime when flipped
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center relative perspective-[2000px] overflow-hidden">
      ...
      >
        {/* --- HOLY GLOW EFFECT (Only show when envelope is closed/opening AND wrapper is ready) --- */}
        {step < 3 && showWrapper && (
