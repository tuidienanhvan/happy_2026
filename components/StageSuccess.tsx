...
const StageSuccess: React.FC<Props> = ({ isEmbedded = false, isActive = true }) => {
  ...
  const handleCelebrate = () => {
    ...
    setTimeout(() => fireRealFirework(0.2, 0.5), 200);
    setTimeout(() => fireRealFirework(0.8, 0.5), 400);
    setTimeout(() => fireRealFirework(0.5, 0.3), 600);
  };

  useEffect(() => {
    // Only auto-fire if active
    if (isActive) {
      const timer = setTimeout(() => fireRealFirework(0.5, 0.5), 800);
  ...
  const Content = (
    <>
      ...
      {/* --- CERTIFICATE CARD --- */}
      <div 
          className={`relative group w-full max-w-[1000px] aspect-[4/5] md:aspect-[3/2] bg-[#fffbf0] shadow-[0_30px_60px_rgba(0,0,0,0.7)] flex flex-col ${!isEmbedded ? 'animate-[tilt-in_1s_ease-out]' : ''}`}
      ...
      >
          ...
          {/* CONTENT CONTAINER */}
          <div className="relative z-10 w-full h-full flex flex-col items-center justify-between p-8 md:p-14 text-center">
              ...
              {/* 3. Body Text Section */}
              <div className="max-w-[90%] space-y-4 md:space-y-6">
                  ...
                  <p className="font-nunito text-xl md:text-2xl text-slate-700 leading-relaxed font-semibold">
              ...
              {/* 5. Footer & Signatures */}
              <div className="w-full grid grid-cols-2 gap-8 mt-6 md:mt-8 px-4 md:px-12 items-end">
                  {/* Date */}
              ...
              {/* 6. The "Ink Stamp" Overlay */}
              <div className={`absolute bottom-16 right-8 md:bottom-24 md:right-16 transform pointer-events-none mix-blend-multiply ${isActive ? 'animate-[stamp-bounce_0.5s_ease-out_1s_forwards] opacity-0' : 'opacity-0'}`}>
      ...
      {/* --- CUSTOM BUTTON (STATIC, ELEGANT) --- */}
      <div className="mt-8 z-50 transform w-full flex justify-center" style={{ backfaceVisibility: 'hidden' }}>
